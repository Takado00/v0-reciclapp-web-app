...
export default async function RecicladorConversacionesPage() {
      ...
      <div className="container py-8">
        <div className="max-w-md mx-auto bg-white dark:bg-gray-800 rounded-lg shadow p-6 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300 mb-6">
            Necesitas iniciar sesión para acceder a tus conversaciones con recicladores.
          </p>
          <div className="flex justify-center gap-4">
            <a
              href="/login?redirect=/mensajes/recicladores"
  ...
  // Filtrar solo las conversaciones con recicladores
  const conversacionesRecicladores =
    conversaciones?.filter((conv) => conv.otroUsuario.tipo_usuario === "reciclador") || []

  return (
    <MensajesLayout>
      {success && conversacionesRecicladores.length > 0 ? (
        <ConversacionesList conversaciones={conversacionesRecicladores} userId={userId} filtroTipo="reciclador" />
      ) : (
        <EmptyState
          title="No tienes conversaciones con recicladores"
          description="Comienza a chatear con recicladores para coordinar la recolección de materiales."
          buttonText="Buscar recicladores"
